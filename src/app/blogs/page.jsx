...
const BlogsPage = () => {
    return (
        <div>
            <p className='text-center font-bold underline border-2 m-4 rounded-2xl border-lime-600 text-blue-700'>blogs page</p>
           {
            blogs.map((blog) => (
                <div key={blog.blog_id} className='border-2 m-3 p-3 '>
                    <p>{blog.blog_name}</p>
                    <p>{blog.description}</p>
                    <button className= 'm-3 btn btn-md border-b-blue-500'><Link href={`/blogs/${blog.blog_name}`}> Details</Link></button>
                </div>
            ))
           }
        </div>
